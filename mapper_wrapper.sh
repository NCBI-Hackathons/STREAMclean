...
do
	SAM_PATH="$WORK_DIR/$SRA_ACC_magicblast.sam"
	if [ -n "$INCLUDE_TAX" ]; then
		"$MAGIC_BLAST_DIR"/bin/magicblast -sra "$SRA_ACC" -db "$BLAST_DB_PATH" -gapextend 0 | \
		python streamin_magicblast.py -m include -s "$SCORE_THRESHOLD" > "$SAM_PATH"
	else
		"$MAGIC_BLAST_DIR"/bin/magicblast -sra "$SRA_ACC" -db "$BLAST_DB_PATH" -gapextend 0 | \
		python streamin_magicblast.py -m exclude -s "$SCORE_THRESHOLD" > "$SAM_PATH"
	fi
done
...
for SRA_ACC in ${SRA_ACCESSIONS//,/ }
do
	cat "$WORK_DIR/$SRA_ACC"_magicblast.sam | python streamin_sam_to_reads.py --score "$MB_SCORE" > "$OUT_DIR/$SRA_ACC"_magicblast.fasta
done
